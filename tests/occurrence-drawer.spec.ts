...
test.describe('OccurrenceDrawer Keyboard Navigation', () => {
  ...
  test('should navigate to next occurrence with right arrow key', async ({ page }) => {
    ...
    const newId = await page.locator('header div:has-text("occurrenceID:")').textContent();

    // Verify occurrence changed
    expect(newId).not.toBe(initialId);
  });

  test('should navigate to previous occurrence with left arrow key', async ({ page }) => {
  ...
  test('should not trigger navigation when PhotoViewer is open', async ({ page }) => {
    ...
    if (hasPhotos) {
      ...
      // Get occurrence ID
      const occurrenceId = await page.locator('header div:has-text("occurrenceID:")').textContent();
  ...
  test('should scroll to occurrence when navigating via next button', async ({ page }) => {
    // Click first occurrence to open drawer
    const firstOccurrence = page.locator('main .occurrence-item').first();
    await firstOccurrence.click();
    await page.waitForSelector('[role="dialog"]', { timeout: 5000 });
  ...
  test('should scroll to occurrence when navigating via previous button', async ({ page }) => {
