<script lang="ts">
  ...
  let emoji = $derived.by(() => {
    ...
    if (item.higherClassification?.toLowerCase()?.match(/decapodiformes/)) return '🦑';
    if (item.higherClassification?.toLowerCase()?.match(/peracarida/)) return '🦐';

    if (item.higherClassification?.toLowerCase()?.match(/marsupialia/)) return '🕷️';
    ...
    if (item.higherClassification?.toLowerCase()?.match(/chelicerata/)) return '🕷️';
