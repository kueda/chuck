<script lang="ts">
import { Move, Pencil, VectorSquare, X } from 'lucide-svelte';
...
function updateResizeHandles(
  ...
  const handleSource = map.getSource(
    'bbox-handles',
  ) as maplibregl.GeoJSONSource;
  if (handleSource) {
      ...
      features: [
        {
          ...
          geometry: { type: 'Point', coordinates: [nelng, nelat] },
        },
        {
          type: 'Feature',
          properties: { corner: 'nw' },
          geometry: { type: 'Point', coordinates: [swlng, nelat] },
        },
        {
          type: 'Feature',
          properties: { corner: 'se' },
          geometry: { type: 'Point', coordinates: [nelng, swlat] },
        },
        {
          type: 'Feature',
          properties: { corner: 'sw' },
          geometry: { type: 'Point', coordinates: [swlng, swlat] },
        },
        {
          type: 'Feature',
          properties: { corner: 'move' },
          geometry: { type: 'Point', coordinates: [centerLng, swlat] },
        },
      ],
    });
  }
...
function updateBoundingBoxLayer(
...
function handleMouseDown(e: maplibregl.MapMouseEvent) {
  ...
  if (handleFeatures.length > 0 && currentBounds) {
    ...
    const corner = handleFeatures[0].properties?.corner as
      | 'ne'
      | 'nw'
      | 'se'
      | 'sw'
