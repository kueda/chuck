...
import { getViewType, saveViewType, getColumnPreferences, saveColumnPreferences } from './viewPreferences';
...
describe('viewPreferences', () => {
  ...
  describe('saveViewType', () => {
    it('saves view type to localStorage', () => {
      saveViewType('cards');
      const stored = JSON.parse(localStorage.getItem('chuck:viewPreferences') || '{}');
  ...
  describe('getColumnPreferences', () => {
    it('returns defaults when no preference saved', () => {
      const result = getColumnPreferences('test-archive', 'id', ['id', 'scientificName', 'eventDate']);
    ...
    it('returns saved column preferences', () => {
      ...
      const result = getColumnPreferences('test-archive', 'id', ['id', 'foo', 'bar']);
